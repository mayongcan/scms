...
public class ScmsTransportInfoRepositoryImpl extends BaseRepository implements ScmsTransportInfoRepositoryCustom{

	private static final String SQL_GET_LIST = "SELECT tb.ID as \"id\", tb.TRANSPORT_NAME as \"transportName\", tb.TYPE as \"type\", tb.MERCHANTS_ID as \"merchantsId\", m.MERCHANTS_NAME as \"merchantsName\" "
			+ "FROM scms_transport_info tb left join scms_merchants_info m on m.ID = tb.MERCHANTS_ID "
			+ "WHERE 1 = 1 ";
	...
	public int getListCount(ScmsTransportInfo scmsTransportInfo, Map<String, Object> params) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST_COUNT, scmsTransportInfo, params);
	 ...
	 * @return
	 */
	private SqlParams genListWhere(String sql, ScmsTransportInfo scmsTransportInfo, Map<String, Object> params){
        ...
        if(!StringUtils.isBlank(name)) {
            sqlParams.querySql.append(getLikeSql("tb.TRANSPORT_NAME", ":name"));
