...
public class ScmsSizeInfoRepositoryImpl extends BaseRepository implements ScmsSizeInfoRepositoryCustom{

	private static final String SQL_GET_LIST = "SELECT tb.ID as \"id\", tb.SIZE_NAME as \"sizeName\", tb.TYPE as \"type\", tb.MERCHANTS_ID as \"merchantsId\", m.MERCHANTS_NAME as \"merchantsName\" "
			+ "FROM scms_size_info tb left join scms_merchants_info m on m.ID = tb.MERCHANTS_ID "
			+ "WHERE 1 = 1 ";
	...
	public List<Map<String, Object>> getList(ScmsSizeInfo scmsSizeInfo, Map<String, Object> params, int pageIndex, int pageSize) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST, scmsSizeInfo, params);
		//添加分页和排序
	...
	public int getListCount(ScmsSizeInfo scmsSizeInfo, Map<String, Object> params) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST_COUNT, scmsSizeInfo, params);
	 ...
	 * @return
	 */
	private SqlParams genListWhere(String sql, ScmsSizeInfo scmsSizeInfo, Map<String, Object> params){
        ...
        if(!StringUtils.isBlank(name)) {
            sqlParams.querySql.append(getLikeSql("tb.SIZE_NAME", ":name"));
