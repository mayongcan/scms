...
public class ScmsTextureInfoRepositoryImpl extends BaseRepository implements ScmsTextureInfoRepositoryCustom{

	private static final String SQL_GET_LIST = "SELECT tb.ID as \"id\", tb.TEXTURE_NAME as \"textureName\", tb.TYPE as \"type\", tb.MERCHANTS_ID as \"merchantsId\", m.MERCHANTS_NAME as \"merchantsName\" "
			+ "FROM scms_texture_info tb left join scms_merchants_info m on m.ID = tb.MERCHANTS_ID "
			+ "WHERE 1 = 1 ";
	...
	public List<Map<String, Object>> getList(ScmsTextureInfo scmsTextureInfo, Map<String, Object> params, int pageIndex, int pageSize) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST, scmsTextureInfo, params);
		//添加分页和排序
	...
	public int getListCount(ScmsTextureInfo scmsTextureInfo, Map<String, Object> params) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST_COUNT, scmsTextureInfo, params);
	 ...
	 * @return
	 */
	private SqlParams genListWhere(String sql, ScmsTextureInfo scmsTextureInfo, Map<String, Object> params){
        ...
        if(!StringUtils.isBlank(name)) {
            sqlParams.querySql.append(getLikeSql("tb.TEXTURE_NAME", ":name"));
