...
public class ScmsCustomerLevelRepositoryImpl extends BaseRepository implements ScmsCustomerLevelRepositoryCustom{

	private static final String SQL_GET_LIST = "SELECT tb.ID as \"id\", tb.LEVEL_NAME as \"levelName\", tb.LEVEL_MEMO as \"levelMemo\", tb.TYPE as \"type\", "
	        + "tb.MERCHANTS_ID as \"merchantsId\", m.MERCHANTS_NAME as \"merchantsName\" "
			+ "FROM scms_customer_level tb left join scms_merchants_info m on m.ID = tb.MERCHANTS_ID "
			+ "WHERE 1 = 1 ";
	...
	public List<Map<String, Object>> getList(ScmsCustomerLevel scmsCustomerLevel, Map<String, Object> params, int pageIndex, int pageSize) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST, scmsCustomerLevel, params);
		//添加分页和排序
	...
	public int getListCount(ScmsCustomerLevel scmsCustomerLevel, Map<String, Object> params) {
		//生成查询条件
		SqlParams sqlParams = genListWhere(SQL_GET_LIST_COUNT, scmsCustomerLevel, params);
	 ...
	 * @return
	 */
	private SqlParams genListWhere(String sql, ScmsCustomerLevel scmsCustomerLevel, Map<String, Object> params){
        ...
        if(!StringUtils.isBlank(name)) {
            sqlParams.querySql.append(getLikeSql("tb.LEVEL_NAME", ":name"));
